...
function createDataset(fields, constraints, sortFields) {
	log.info("arquivo_kpi_atraso_entrega - INICIO ##########");
...
DatasetFactory.getDataset('arquivo_kpi_atraso_entrega',[],cs,[]);
 */

function montaBusca(constraints){
	log.info("arquivo_kpi_atraso_entrega - montaBusca - constraints: ");
	log.dir(constraints);
	...
	if (constraints != null && constraints.length > 0 && constraints[0].fieldName != "sqlLimit") {
		where = " WHERE ((et.status = 'Pendente' AND dDocumento.dt_criacao is NULL AND et.dtPrevista < CURDATE())";
		where += "|| (dDocumento.dt_criacao IS NOT NULL AND et.dtPrevista < dDocumento.dt_criacao))";
