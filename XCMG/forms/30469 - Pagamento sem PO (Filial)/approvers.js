let APPROVERS = {
	...
	*/
	...
	, setGenericApprovers: function(data){
		...
		$("#nm_funcao_4").val("Diretor Geral 领导");
		if(data && data.diretor_geral != null) {
			$("#cd_diretor_geral").val(data.diretor_geral.cd);
			$("#nm_diretor_geral").val(data.diretor_geral.nm);
		}
		...
		if(data && data.presidencia_filial != null) {
			$("#cd_presidencia").val(data.presidencia_filial.cd);
			$("#nm_presidencia").val(data.presidencia_filial.nm);
		}
	}
