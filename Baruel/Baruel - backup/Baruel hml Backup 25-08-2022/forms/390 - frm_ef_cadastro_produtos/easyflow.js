 ...
 */
 function isFormFillState() {
	...
	const formFillState = getFormFillState();
	return (state == formFillState) || (state == 0) || (state == 4);
}
...
function enableTabs() {
	...
	dsParTabs.values.forEach((par, idx) => {
		...
		for (let i = 0; i < dsTabs.values.length; i++) {
			...
			let currentTab = $('#dynamicTabs a').filter(function () {
				return $(this).text() == v.nomeTab
			});
...
function beforeSendValidate(numState, nextState) {
	...
	for (let i = 0; i < requiredInputs.length; i++) {
		...
		if (
			...
			if (inputName = 'B1_CODGTIN' && $("#B1_CODGTIN").val().length < 15){
				
				console.log("length = " +("#B1_CODGTIN").val().length);
				
				errorMessage += 'Campo Cód. GTIN não pode possuir menos de 15 caracteres! \n';		
			}	
...
function allowEnglishCharsOnly(elem) {
	// Impede caracteres especiais logo ao digitar
	...
	// Troca caracteres especiais ao digitar ou colar
	elem.blur(function (event) {
		const text = event.target.value;
		let buffer = "";
		for (let i = 0; i < text.length; i++) {
			buffer += handleSpecialChar(text[i]);
		}
...
/**
 * Retorna um caracter equivalente seguro, caso seja especial.
 * @param {string} char 
 * @returns string
 */
