function validateForm(form) {
	...
	else if (periodicidade == "mensalAvulso"){
		var table = form.getChildrenIndexes('tabelaDatas');

		if (table.length == 0) {
			throw "Adicione pelo menos uma data!"
		} else {
			for (var i = 0; i < table.length; i++) {
				if (form.getValue('quandoAvulso___' + table[i]) == '') {
					throw "Selecione a data !"
				}
			}
		}
	}
