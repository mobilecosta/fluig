...
function init() {
	...
	// aprovacao gestores
		...
		if (document.getElementById("areaSolic").value == "8" || document.getElementById("areaSolic").value == "9" || document.getElementById("areaSolic").value == "10") {
			var aprovadorDaVez;
			$("input[id^='idEmailAprovador___']").each(function (i, input) {
				if (mailCurrentUser == input.value) {
					aprovadorDaVez = i + 1;
				}
			});

			document.getElementById('hiddenNumeroDaVez').value = aprovadorDaVez;
		}
	...
	if (mode != "VIEW" && (numeroAtividade == "0" || numeroAtividade == "" || numeroAtividade == null || numeroAtividade == "1" || numeroAtividade == "6")) {
		...
		if (fValorBruto != 0) {
			...
			if (document.getElementById("areaSolic").value != "x" && document.getElementById("areaSolic").value != "4" && document.getElementById("areaSolic").value != "5" && document.getElementById("areaSolic").value != "6" && document.getElementById("areaSolic").value != "7") {
				...
				if (document.getElementById("areaSolic").value == "8") {

					var aprovadores = getAprovadoresAcordoJudicial(fValorBruto);


					document.getElementById("hiddenAprovadorDaVez").value = aprovadores['aprovador_trabalhista'].split("\u0018").join(',');
					cPapel = "Aprovadores Acordos Judiciais Trabalhista";
					lEncontrouAreasExcecao = true; // ENCONTROU - ENTAO UTILIZAR ESTA REGRA
				}

				if (document.getElementById("areaSolic").value == "9") {

					var aprovadores = getAprovadoresAcordoJudicial(fValorBruto);


					document.getElementById("hiddenAprovadorDaVez").value = aprovadores['aprovador_civel'].split("\u0018").join(',');
					cPapel = "Aprovadores Acordos Judiciais Civeis";
					lEncontrouAreasExcecao = true; // ENCONTROU - ENTAO UTILIZAR ESTA REGRA
				}

				if (document.getElementById("areaSolic").value == "10") {

					var aprovadores = getAprovadoresAcordoJudicial(fValorBruto);


					document.getElementById("hiddenAprovadorDaVez").value = aprovadores['aprovador_tributario'].split("\u0018").join(',');
					cPapel = "Aprovadores Acordos Judiciais Tributarios";
					lEncontrouAreasExcecao = true; // ENCONTROU - ENTAO UTILIZAR ESTA REGRA
				}
				...
				if (lEncontrouAreasExcecao == true) {
					...
					if (document.getElementById("areaSolic").value == "8" || document.getElementById("areaSolic").value == "9" || document.getElementById("areaSolic").value == "10") {
						...
						var cHidden = document.getElementById("hiddenAprovadorDaVez").value.split(',');

						console.log(cHidden);

						for (var i = 0; i <= cHidden.length - 1; i++) {
							var index = wdkAddChild("tbAprovacoes");
							var c1 = DatasetFactory.createConstraint("mail", cHidden[i], cHidden[i], ConstraintType.MUST);
							var constraints = new Array(c1);
							var resultado = DatasetFactory.getDataset("colleague", null, constraints, null);
							var matricula = resultado.values[0]["colleaguePK.colleagueId"];
							var nomeAprovador = resultado.values[0]["colleagueName"];
							$("#idMatrAprovador___" + index).val(matricula);
							$("#idEmailAprovador___" + index).val(cHidden[i]);
							$("#idNomeAprovador___" + index).val(nomeAprovador);
...
function getAprovadoresAcordoJudicial(valor) {

	var constraint = [];
	constraint.push(DatasetFactory.createConstraint("metadata#active", true, true, ConstraintType.MUST));
	constraint.push(DatasetFactory.createConstraint("tablename", "aprovadores", "aprovadores", ConstraintType.MUST));
	var dataset = DatasetFactory.getDataset("aprovadores_acordos_judiciais", null, constraint, null);


	for (var i = 0; i < dataset.values.length; i++) {
		var valorde = parseFloat(dataset.values[i].valorde.split('.').join(''));
		var valorpara = parseFloat(dataset.values[i].valorpara.split('.').join(''));

		if (valor >= valorde && valor <= valorpara) {
			return dataset.values[i];
		}
	}

	return dataset.values[dataset.values.length - 1];
}
